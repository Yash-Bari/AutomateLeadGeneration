...
def generate_personalized_email_content(lead, location, business_type):
    response = openai.Completion.create(
        ...
        # we can make prompt according to requirement of mail
...
def main():
    ...
    business_type = input("Enter the type of industry: ")
    total = int(input("Enter the total number of listings to scrape: "))
    spreadsheet_key = 'your_key'  # Replace with your Google Sheets key
    ...
    if business_list:
        ...
        for lead in business_list.business_list:
            ...
            print(f"Generated Email for {lead.name}:")
            print(email_content)

            send_emails = input(f"Do you want to send this email to {lead.name}? (yes/no): ")
            ...
            if send_emails.lower() == 'yes':
                ...
                if send_email(smtp_server, smtp_port, smtp_username, smtp_password, smtp_username, lead.email,
                    ...
                    print(f"Email sent successfully to {lead.name}")
                else:
